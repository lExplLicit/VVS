<?php
session_start();
$init = json_decode(file_get_contents('install.json'), true);
if (!$init['INIT']) {
...
?>
  ...
  <form action="installscript.php?step=2" method="POST">
...
<?php
} elseif (($_GET['step'] == 2) && isset($_POST['server']) && isset($_POST['benutzer']) && isset($_POST['passwort']) && isset($_POST['name'])) {
  ...
  if ($link === false) {
    ...
    echo "Verbindung erfolgreich.. Zugangsdaten wurden gespeichert..<br><br>";
    $_SESSION['csfr'] = rand(10000, 50000);

    echo '<form action="installscript.php?step=89e5r1f68sdf4v" method="POST"><input type="hidden" name="csrf" value="' . $_SESSION['csfr'] . '"><input type="submit" value="Nächster Schritt"></form>';
    die();
  }
} elseif ($_GET['step'] == "89e5r1f68sdf4v") {
  ...
  echo "Initialisierung der Datenbank..<br>";
  echo "Anlegen der Tabellen..<br>";
  echo "Benutzer Hinzufügen..<br>";
  ...
  echo "<br>Installation Erfolgreich..Bitte Administrator Anlegen<br><br>";

?>
  <form action="installscript.php?step=45ydt267jh54" method="POST">
    <label>Admin Vorname:</label><br>
    <input type="text" name="vorname" required><br>
    <label>Admin Nachname:</label><br>
    ...
    <label for="exam">Beispieldaten hinzufügen?</label>
    <br><br>
    <input type="submit" value="Bestätigen">
  </form>
...
(NULL, \'mcobbin1g@squidoo.com\', \'87ba872fca78f06d13d8feda35da06bbaeef61ee9d0b8bea04b891aadb8ba993\', 0, 0, \'Mervin\', \'Cobbin\', \'Katz\', \'2020-02-18 15:29:11\');');
  ...
  echo '<form action="../index.php?" method="GET"><input type="submit" value="Abschluss"></form>';
  die();
}
